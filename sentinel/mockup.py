...
model_path = '/vol/bitbucket/sww116/msc-ai-group-project_frontend/sentinel/sentiment_analysis/saved_model_new/bert-base-uncased_final_1.pt'
...
def index():
    ...
    if request.method == 'POST':
        ...
        print("cstart",file=sys.stderr)
        keyword = request.form["input"]
        print("got keyword", file=sys.stderr)
...
def advanced():
    if request.method == 'POST':
        ...
        print(tweet_type, file=sys.stderr)
        print(num_topics, file=sys.stderr)
        print(num_tweets, file=sys.stderr)
        if not keyword:
            keyword = 'cat'
